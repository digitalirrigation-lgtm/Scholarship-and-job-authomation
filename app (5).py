# app.py – FINAL ALTAIR VERSION (NO PLOTLY)
...
def ensure_table_schema():
    ...
    needed = {"GeneratedCV": "TEXT", "GeneratedCL": "TEXT", "GeneratedML": "TEXT", "AppliedTimestamp": "TEXT"}
...
def reset_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS Opportunities (
        ...
        GeneratedCV TEXT, GeneratedCL TEXT, GeneratedML TEXT, AppliedTimestamp TEXT
    )''')
...
# ---------- FUNCTIONS ----------
def fetch_all():
    conn = get_db(); df = pd.read_sql("SELECT * FROM Opportunities ORDER BY Id DESC", conn); conn.close(); return df

def fetch_profile():
    conn = get_db(); df = pd.read_sql("SELECT * FROM MasterProfile LIMIT 1", conn); conn.close(); return df
...
def update_generated_docs(opp_id, cv, cl, ml):
    conn = get_db(); c = conn.cursor()
    c.execute("UPDATE Opportunities SET GeneratedCV=?, GeneratedCL=?, GeneratedML=? WHERE Id=?", (cv, cl, ml, opp_id))
    conn.commit(); conn.close()
...
def generate_text(prompt, max_length=512):
    if not USE_LOCAL_AI or not AI_AVAILABLE: return None
...
def open_browser(link):
    try:
        ...
        driver.get(link); st.success("🌐 Browser opened."); return driver
    except Exception as e:
        st.error(f"Browser error: {e}"); return None

# ---------- UI ----------
st.set_page_config(layout="wide", page_title="🎓 AI Dashboard")
st.markdown("""
<style>
.stApp { background: linear-gradient(180deg, #1a1a2e 0%, #16213e 30%, #0f3460 60%, #533483 100%); }
.golden-text { color: #FFD700; text-shadow: 0 0 10px rgba(255,215,0,0.5); }
.stButton button { background: linear-gradient(145deg, #FFD700, #B8860B) !important; color: #1a1a2e !important; border-radius: 30px !important; border: none !important; font-weight: bold !important; box-shadow: 0 4px 15px rgba(255,215,0,0.4) !important; }
.stButton button:hover { transform: scale(1.05); }
</style>""", unsafe_allow_html=True)

st.title("🎓 Scholarship & Job AI Dashboard")
st.markdown("<p class='golden-text' style='font-size:1.2rem;'>Your own local AI – Golden Theme</p>", unsafe_allow_html=True)
...
if not df_all.empty:
    ...
    for _, row in urgent.iterrows(): st.sidebar.write(f"• {row['Title']} ({row['DaysLeft']} days left)")
    st.sidebar.markdown("### 🟡 Upcoming (11–30 days)")
    for _, row in upcoming.iterrows(): st.sidebar.write(f"• {row['Title']} ({row['DaysLeft']} days left)")
    st.sidebar.markdown("### 🟢 Safe (>30 days)")
    for _, row in safe.iterrows(): st.sidebar.write(f"• {row['Title']} ({row['DaysLeft']} days left)")
else:
...
df = fetch_all()
if not df.empty:
    total = len(df); applied = len(df[df['Status'] == 'Applied']); pending = len(df[df['Status'] == 'Not Applied'])
    urgent_count = len(df[pd.to_datetime(df['Deadline']) <= datetime.today() + timedelta(days=10)])
    col1.metric("📌 Total", total); col2.metric("✅ Applied", applied); col3.metric("⏳ Pending", pending); col4.metric("🔴 Urgent", urgent_count, delta="action needed" if urgent_count>0 else None)
else:
    col1.metric("📌 Total", 0); col2.metric("✅ Applied", 0); col3.metric("⏳ Pending", 0); col4.metric("🔴 Urgent", 0)
...
if not df.empty:
    ...
    if not applied_df.empty:
        ...
        daily.columns = ['Date', 'Daily']; daily = daily.sort_values('Date'); daily['Cumulative'] = daily['Daily'].cumsum()
        col_c1, col_c2 = st.columns(2)
        with col_c1:
            bar = alt.Chart(daily).mark_bar(color='gold', opacity=0.7).encode(x='Date:T', y='Daily:Q').properties(title='Daily Applications', height=300)
            st.altair_chart(bar, use_container_width=True)
        with col_c2:
            line = alt.Chart(daily).mark_line(point=True, color='silver').encode(x='Date:T', y='Cumulative:Q').properties(title='Cumulative Total', height=300)
        ...
        hour_chart = alt.Chart(hour_counts).mark_bar(color='gold', opacity=0.8).encode(x='Hour:O', y='Count:Q').properties(title='Applications by Hour of Day', height=300)
    ...
    if selected_id:
        ...
        if profile_df.empty:
            ...
            with st.expander(f"📄 {row['Title']} – {row['Organization']}", expanded=True):
                ...
                with col_gen:
                    if st.button("⚡ Generate Documents (AI)"):
                        with st.spinner("Generating..."):
                            ...
                            st.success("✅ Generated and saved!"); st.rerun()
                with col_status:
                    if st.button("✅ Mark as Applied"):
                        update_status(selected_id, "Applied"); st.rerun()
                with col_del:
                    if st.button("🗑️ Delete"):
                        delete_opportunity(selected_id); st.rerun()
                with col_browser:
                ...
                if row['GeneratedCV']:
                    st.subheader("📄 CV"); st.text_area("CV", row['GeneratedCV'], height=200)
                    st.download_button("⬇️ Download CV", data=row['GeneratedCV'], file_name=f"CV_{row['Title']}.txt")
                if row['GeneratedCL']:
                    st.subheader("✉️ Cover Letter"); st.text_area("Cover Letter", row['GeneratedCL'], height=200)
                    st.download_button("⬇️ Download Cover Letter", data=row['GeneratedCL'], file_name=f"CL_{row['Title']}.txt")
                if row['GeneratedML']:
                    st.subheader("📨 Motivation Letter"); st.text_area("Motivation Letter", row['GeneratedML'], height=200)
